...
def mask_subtract_arr(image: ArrayLike, mask: ArrayLike, return_nan=True):
    ...
    if return_nan:
        return np.where(masked_image == 0, np.nan,masked_image)
    return masked_image
...
#Functions for roughness calculation
@njit
def calc_ra(image,normalize=False):
...
# @njit(parallel=True)
# def batch_level_Ra(image: np.array, tileshape: tuple, imageshape: tuple, threshold, progressupdate=True):
#     """efficient numba implementation of a function that takes a 3d image as a np array, tiles it, levels it, 
#     and segments it """
#     segmented_timeseries = np.zeros(len(image))
